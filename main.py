import pygame, sys

pygame.init()

# colors
BLACK = (0, 0, 0)
WHITE = (255, 255, 255)
SKY_BLUE = (135, 206, 235)

WINDOW_SIZE = (640, 480)

window = pygame.display.set_mode(WINDOW_SIZE)
pygame.display.set_caption("My Platform")

DISPLAY_SIZE = (80, 60)
display = pygame.Surface(DISPLAY_SIZE)

time = pygame.time.Clock()
loop = True


# map
def load_map(path):
    f = open('maps/' + path + '.txt', 'r')
    data = f.read()
    f.close()
    data = data.split('\n')
    game_map = []
    for row in data:
        game_map.append(list(row))
    return game_map


MAP_1 = load_map('map1')
main_map = MAP_1

TILE_SIZE = 8

# images
player_img = pygame.image.load("assets/player.png")
grass_img = pygame.image.load("assets/grass.png")
dirt_img = pygame.image.load("assets/dirt.png")

player_rect = pygame.Rect(35, 10, player_img.get_width(), player_img.get_height())

player_move_right = False
player_move_left = False

player_y_momentum = 0

X_VEL = 1

jump = False


def collision_test(rect, tiles):
    hit_list = []
    for tile in tiles:
        if rect.colliderect(tile):
            hit_list.append(tile)
    return hit_list


def move(rect, movement, tiles):
    collision_types = {'top': False, 'bottom': False, 'right': False, 'left': False}

    rect.x += movement[0]
    hit_list = collision_test(rect, tiles)
    for tile in hit_list:
        if movement[0] > 0:
            rect.right = tile.left
            collision_types['right'] = True
        elif movement[0] < 0:
            rect.left = tile.right
            collision_types['left'] = True

    rect.y += movement[1]
    hit_list = collision_test(rect, tiles)
    for tile in hit_list:
        if movement[1] > 0:
            rect.bottom = tile.top
            collision_types['bottom'] = True
        elif movement[1] < 0:
            rect.top = tile.bottom
            collision_types['top'] = True
    return rect, collision_types


# camera
scroll = [0, 0]

while loop:
    display.fill(SKY_BLUE)

    scroll[0] += (player_rect.x - scroll[0] - 40) / 5
    scroll[1] += (player_rect.y - scroll[1] - 30)/2

    if scroll[1] >= 10:
        scroll[1] = 10

    # tilemap
    tile_rect = []
    y = 0
    for row in main_map:
        x = 0
        for tile in row:
            if tile == '2':
                display.blit(grass_img, (x * TILE_SIZE - scroll[0], y * TILE_SIZE - scroll[1]))
            if tile == '1':
                display.blit(dirt_img, (x * TILE_SIZE - scroll[0], y * TILE_SIZE - scroll[1]))
            if tile != '0':
                tile_rect.append(pygame.Rect(x * TILE_SIZE, y * TILE_SIZE, TILE_SIZE, TILE_SIZE))
            x += 1
        y += 1

    player_movement = [0, 0]
    # move
    if player_move_right:
        player_movement[0] += X_VEL
    if player_move_left:
        player_movement[0] -= X_VEL
    # gravity
    player_movement[1] += player_y_momentum
    player_y_momentum += 0.2
    if player_y_momentum > 2:
        player_y_momentum = 2

    player_rect, collisions = move(player_rect, player_movement, tile_rect)

    if collisions['bottom']:
        player_y_momentum = 1
        jump = True
    if not collisions['bottom']:
        jump = False
    if collisions['top']:
        player_y_momentum = 0.5

    # change map

    display.blit(player_img, (player_rect.x - scroll[0], player_rect.y - scroll[1]))

    # input
    for event in pygame.event.get():  # event loop
        if event.type == pygame.QUIT:  # check for window quit
            pygame.quit()  # stop pygame
            sys.exit()  # stop script
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_RIGHT:
                player_move_right = True
            if event.key == pygame.K_LEFT:
                player_move_left = True
            if event.key == pygame.K_UP:
                if jump:
                    player_y_momentum = -2.1
                    jump = False
        if event.type == pygame.KEYUP:
            if event.key == pygame.K_RIGHT:
                player_move_right = False
            if event.key == pygame.K_LEFT:
                player_move_left = False

    window.blit(pygame.transform.scale(display, WINDOW_SIZE), (0, 0))
    pygame.display.update()
    time.tick(60)
